...
Verifies the minimum next-turn visibility of `LAST_SKILL_USE_RESULTS`. Output of a skill call in turn N is exposed to the LLM at turn N+1 and remains task evidence until new input resets it. The test does not require the agent to "behave intelligently"; it confirms the carry exists.
